...
class Wallet {
  ...
  static sendTransaction(
    ...
    const wallet = new this(_receivedTx.sender, _receivedTx.signature, _utxo);
    ...
    const _myUTXO = UnspentTxOut.getMyUTXO(wallet.address, _utxo);
    console.log(" 6-22 트랜잭션 생성 함수 호출");

    const tx = Transaction.createTx(_receivedTx, _myUTXO);
