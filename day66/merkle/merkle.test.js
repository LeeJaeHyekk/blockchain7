...
//  - npm jest 를 실행하면 .test .js 파일을 모두 확인한다.
//  - 또는 package.json 파일안에 test 명령어를 jest로 수정한다.
//      - "test" : "jest"
...
//   //  - toXXXXXX 를 매쳐(matcher)라고 부른다.
//   //  - toBe의 경우 완벽한 일치를 확인하기 떄문에 객체 등을 비교할 수 는 없다.
//   it("이건 안된다 -> 이건 된다.", () => {
//     // expect({ a: 1 }).toBe({ a: 1 });
//     //   객체는 서로 다른 것으로 취급한다. 같은 객체가 되려면 변수에 정의해야 한다.
//     // const obj = { a: 1 };
//     // expect(obj).toBe(obj);
//   });
...
//       //   toStrictEqual은 내용 자체가 모두 일치해야 한다. 같아야 한다.
//     });
//   });
// });
describe("merkle 비교", () => {
  it("하나의 데이터 암호화 확인", () => {
    ...
    // 배열내의 데이터가 숫자! 1 이기 떄문에 현재는 오류가 발생한다.
  });
  it("두개의 데이터 암호화 확인", () => {
