...
const createMerkleRoot = () => {
  return SHA256(`${data[0]}`).toString();
  //   SHA256(data[0]) << 데이터를 암호화한다. 해당 암호화된 데이터는 객체 형식이다.
  //    toString() << 문자열로 변환한다.
  //   .toUpperCase() << SHA256를 쓸 경우 영어가 소문자로 나타나기 떄문에 대문자로 수정해 준다.
};
const firstTree = [];
...
const thirdTreeRoot = SHA256(secondTree[0] + secondTree[1])
  .toString()
  .toUpperCase();
//   데이터(data) 안의 2개의 아이템을 암호화한 후 연결해서 다시 암호화 하면 한개의 문자열로 만든다.
module.exports = {
  merkleRoot,
...
// 머클 라이브러리는 받은 데이터 배열의 아이템을 전부 문자열로 처리한다.
